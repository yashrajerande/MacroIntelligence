...
// prevOffset: which prior observation to use as `previous` (default 1 = the
//   adjacent one). WALCL is a WEEKLY series while the schema treats the
//   balance sheet as monthly — offset 4 makes the delta month-over-month
//   instead of a week-over-week move presented as the monthly change.
const SERIES_MAP = {
  us_10y_treasury:  { id: 'DGS10' },
  fed_funds_rate:   { id: 'FEDFUNDS' },
  fed_balance_sheet:{ id: 'WALCL', transform: v => v / 1000, prevOffset: 4 },  // $ millions → $ billions
...
async function fetchSeries(spec, apiKey) {
  ...
  const prevOffset = spec.prevOffset || 1;
  // Fetch a few extra observations: business-day series return "." for
  // holidays, which we must skip rather than turn into NaN.
  const limit = prevOffset + 4;
  const url = `https://api.stlouisfed.org/fred/series/observations?series_id=${spec.id}&sort_order=desc&limit=${limit}&api_key=${apiKey}&file_type=json${unitsParam}`;
  ...
  try {
    ...
    // FRED encodes missing observations (e.g. DGS10 on a US holiday) as
    // ".", which parseFloat turns into NaN — previously that NaN shipped
    // as a real value with no fetch_error and poisoned the cache.
    const obs = (data.observations || []).filter(o => Number.isFinite(parseFloat(o.value)));
    if (obs.length === 0) throw new Error('No numeric observations');

    const xform = spec.transform || (v => v);
    const round4 = v => Math.round(v * 10000) / 10000;
    const latest = round4(xform(parseFloat(obs[0].value)));
    const previous = obs.length > prevOffset ? round4(xform(parseFloat(obs[prevOffset].value))) : latest;
